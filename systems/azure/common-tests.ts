...
const generateRawLicenseData = (systemData: any) => {
  const data: { licenses: any[]; hasNecessaryLicenses: boolean } = {
    licenses: [],
    hasNecessaryLicenses: false
  };

  data.licenses = systemData.assignedLicenses.map((license: any) => {
    const lic = licenses.find((l) => l.skuId === license.skuId);
    if (lic) {
...
export const azureAktiveringAnsatt = {
...
export const azureAktiveringElev = {
  ...
  test: (user: any, systemData: any, allData: any) => {
    ...
    const data = {
      ...
      vis: {
        active: allData["fint-elev"].elevforhold.find((forhold: any) => forhold.aktiv || new Date() < new Date(forhold.gyldighetsperiode.start))
      }
    };
...
export const azureUpnEqualsMail = {
  id: "azure_equal_mail",
...
export const azureLicense = {
  ...
  test: (_user: any, systemData: any) => {
    ...
    if (data.hasNecessaryLicenses) return success({ message: "Har Microsoft 365-lisenser", solution: data.licenses.map((lic: any) => lic.name || lic.skuId).join(", "), raw: data });
...
export const azureLicenseDowngrade = {
  ...
  test: (_user: any, systemData: any) => {
    ...
    const { licenses: userLicenses } = generateRawLicenseData(systemData);
    return warn({
...
export const azureLicenseA1 = {
  ...
  test: (user: any, systemData: any) => {
    ...
    if (!userLicenses.some((license: any) => license.name?.includes(" A1 "))) {
      return ignore();
    }
...
export const azureLicenseManuallyChanged = {
...
export const azurePwdKluss = {
  ...
  test: (_user: any, systemData: any) => {
    const data = {
      userSignInErrors: systemData.userSignInErrors.filter((err: any) => err.status.errorCode === 50126)
    };
...
export const azureProxyAddresses = {
...
export const azureGroups = {
  ...
  test: (_user: any, systemData: any) => {
    ...
    const groups = {
      regular: systemData.memberOf.filter((group: string) => !group.trim().startsWith("MEM-User-")),
      mem: systemData.memberOf.filter((group: string) => group.trim().startsWith("MEM-User-"))
    };
...
export const azureConditionalAccessPersonaGroup = {
  ...
  waitForAllData: false,
  test: (_user: any, systemData: any) => {
    const conditionalAccessPersonaGroups = systemData.memberOf.filter((group: string) => group.trim().toLowerCase().startsWith("conditional access persona"));
...
export const azureRiskyUser = {
  ...
  test: (user: any, systemData: any) => {
    ...
    if (data.riskyUser.length > 0)
      return error({
        message: `Brukeren har havna i risky users, på nivå ${data.riskyUser.map((risk: any) => risk.riskLevel).join("og ")} 😱`,
