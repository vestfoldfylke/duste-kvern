const validateDate = (digits) => {
  ...
  return date && date.getMonth() + 1 === parseInt(month, 10) && date.getDate() === parseInt(day, 10);
};
...
const validateChecksum = (digits) => {
  ...
  return k1 < 10 && k2 < 10 && k1 === parseInt(digits[9], 10) && k2 === parseInt(digits[10], 10);
};
 ...
 */
const isValidFnr = (digits) => {
  if (!digits || digits.length !== 11) return { valid: false, error: "Fødselsnummeret har ugyldig lengde" };

  const isDnr = parseInt(digits.substring(0, 1), 10) > 3;
  const isVnr = isDnr && parseInt(digits.substring(6, 8), 10) === 99;
